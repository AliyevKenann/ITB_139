...
const foto24 = document.getElementById("foto24")
...
searchInput.addEventListener("keyup",(e)=>{
    let filteredList = data.filter(item => {
        ...
        if (ticker.includes(value) || ticker.includes(value)){
            return item
        }
    })
    if (filteredList.length !== 0){
    tableRender(filteredList);
    } else {
        foto24.style.display = "block"
    }
    ...
    function tableRender(param){
        ...
        param.forEach(item => {
            ...
            <td>${item.ticker}</td>
            <td>${item.value}</td>
            <td class="${item.change[0] === "-" ? "red" : "green"}">
            ${item.change}
            </td>
</tr>`
        })
